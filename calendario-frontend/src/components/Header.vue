<template>
    <nav class="flex items-center justify-between flex-wrap sm:px-10 py-5 px-6 print:hidden">
        <div class="left flex items-center">
            <label class="btn btn-circle btn-ghost swap swap-rotate inline-grid md:hidden">
                <input type="checkbox"/>
            ...
            <router-link v-if="$store.state.token_exist" to="/labs" class="uppercase hover:text-primary mr-6 hidden md:inline-block text-align:center">Labs</router-link>
            <router-link v-if="$store.state.token_exist" to="/instructors" class="uppercase hover:text-primary mr-6 hidden md:inline-block text-align:center">Instructores</router-link>
        </div>

        <div class="flex items-center">
            <a class="uppercase hover:text-primary hidden sm:inline-block text-align:center mr-6" href="/dashboard">TABLERO</a>
