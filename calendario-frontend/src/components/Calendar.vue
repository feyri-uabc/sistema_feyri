<template>
    <div class="calendar">
        <div :class="(!$store.state.expanded_calendar?'my-4':'')">
            <div v-if="!$store.state.expanded_calendar">
            ...
            <div :class="'flex items-center justify-between print:hidden ' + ($store.state.expanded_calendar ?'' :'md:justify-end')">
                ...
                <slot v-if="$store.state.expanded_calendar" name="subtitle"/>
        ...
        <div class="calendar-content">
            <div class="time">
                ...
                <div :class="'box ' + ($store.state.expanded_calendar?'show-maximized':'')" v-for="time in (end_time - start_time)">
            ...
            <div class="days px-2">
                <div class="row-days" v-for="(day, index) in days_week" :title="day.name">
                    ...
                    <div v-for="time in (end_time - start_time)" :id="createDateID(day, time)" :class="'box ' + ($store.state.expanded_calendar?'show-maximized':'')"/>
                </div>
            </div>
        </div>
    </div>
...
.show-maximized {
    max-height: calc((100vh - 20rem) / 16);
}
